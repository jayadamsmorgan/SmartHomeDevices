...
void update_EEPROM_variables(int id, bool on, uint8_t brightness) {
...
void parse_udp_packet(AsyncUDPPacket packet) {
  ...
  // brightness parsing
  String brightnessString = parse_string_from_payload(packet_string, "brightness");
  if (brightnessString.equals("")) {
    return;
  }
  uint8_t brightness = brightnessString.toInt();
  log("Successfully parsed UDP Packet from " + packet.remoteIP().toString() + ".");
  update_EEPROM_variables(id, on, brightness);
}
...
void send_current_state_to_server() {
  ...
  state_string = state_string + ";brightness=" + state.brightness;
...
void loop() {
  ...
  ledcWrite(PWM_CHANNEL, state.isOn ? state.brightness : 0);
